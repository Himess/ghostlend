...
To decode a single log instead of a batch, use the individual decoders (`decodeConfidentialTransfer(log)`, `decodeWrap(log)`, …), each of which returns `null` for a non-matching log. See the [event decoders reference](/protocol/sdk/alpha/api-references/sdk/event-decoders.md) for the full list and field types.
...
If you only ever index your own accounts' activity, you can stop here. If you need to decrypt on behalf of other users, follow the [Delegated decryption](/protocol/sdk/alpha/guides/delegated-decryption.md) guide — the only change to the loop above is swapping `decryptValues` for `delegatedDecryptValues` and passing the delegator's address.
...
Gate the decrypt behind a permit check so the wallet signature prompt only appears on user action — see [Encrypt & decrypt](/protocol/sdk/alpha/guides/encrypt-decrypt.md#3-decryption-of-the-encrypted-data) for the `DecryptGate` pattern.

## Next steps

* [Event decoders](/protocol/sdk/alpha/api-references/sdk/event-decoders.md) — every decoder, finder, and event field type
* [Node.js backend](/protocol/sdk/alpha/guides/node-js-backend.md) — backend setup, storage isolation, and direct API key auth
* [Delegated decryption](/protocol/sdk/alpha/guides/delegated-decryption.md) — decrypt on behalf of other users
* [Encrypt & decrypt](/protocol/sdk/alpha/guides/encrypt-decrypt.md) — the full decryption UX in the browser
